...
export async function login(req, res) {
    try {
        ...
        if (!email || !password) return res.status(400).json({  error: 'Name and email are required' });

        const lookupUser = await prisma.user.findUnique({
            where: { email: email }
        });
        if (!lookupUser) return res.status(404).json({ error: 'No user found with that email, please try again.' });
        const correctPassword = await bcrypt.compare(password, lookupUser.password);
        if (!correctPassword) return res.status(400).json({ error: 'Password is incorrect for this email, please try again.'})
        ...
        console.error(error);
        res.status(500).json({ error: 'Server error, failed to login, please try again.'  })
    }
}
